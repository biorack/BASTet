...
class omsi_filter_by_mask(analysis_base):
    ...
    def __init__(self, name_key="undefined"):
        ...
        super(omsi_filter_by_mask, self).__init__()
        ...
        self.add_parameter(name='mask',
                           ...
                           required=True,
                           group=groups['input'])
        self.data_names = ['msidata','mzdata']

    def execute_analysis(self):
        # Assign parameters to local variables for convenience
        current_msidata = self['msidata']
        current_mzdata = self['mzdata']
        current_mask = self['mask']
        mzdata = current_mzdata[:]
        msidata = current_msidata[:]
        msidata = np.multiply(msidata, current_mask[:,:,np.newaxis])

        return msidata, mzdata
...
if __name__ == "__main__":
    ...
    omsi_cl_driver(analysis_class=omsi_filter_by_mask).main()
