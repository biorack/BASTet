...
###############################################################
class omsi_ticNorm(omsi_analysis_base) :
    ...
    def __init__(self, nameKey="undefined"):
        ...
        super(omsi_ticNorm,self).__init__()
    ...
    def execute_analysis(self) :
        ...
        """
        Nx = self['msidata'].shape[0]
        Ny = self['msidata'].shape[1]
        Nz = self['msidata'].shape[2]
        ...
        if ionList is not None:
            ...
            idx_mz = range(Nz)
        # ticMask = np.zeros( (imTIC.shape[0],imTIC.shape[1]) )
        # idx = np.where ( np.amax(msidata,2) >= self['maxCount'] )
        # ticMask[idx] = 1

        # the numbers are so small that double precision is necessary
        # self['norm_msidata'] = np.divide(np.multiply(msidata,ticMask[:,:,np.newaxis]).astype(float),imTIC.astype(float)[:,:,np.newaxis])
        ...
        #
        idx = np.linspace(0,Nx,20).astype(int)
        #
        # # ofp = ofp.reshape(Nx,Ny,Nz)
        #
        for i in range(len(idx)-1):
            tempData = self['msidata'][idx[i]:idx[i+1],:,:]
            Np = idx[i+1]-idx[i]
            tempData = tempData.reshape(Np*Ny,Nz)
            tip = np.sum(tempData[:,idx_mz],1)
            mip = np.amax(tempData[:,idx_mz],1)
            idx_thresh = np.where ( (mip >= self['maxCount']) & (tip>0) )
            tempNorm = np.zeros(tempData.shape)
            if len(idx_thresh[0])>0:
                tempNorm[idx_thresh[0],:] = np.divide(tempData.astype(float)[idx_thresh[0],:],tip.astype(float)[idx_thresh[0],np.newaxis])
                ofp = np.memmap(outputFilename, dtype='float', mode='w+', shape=(Nx,Ny,Nz))
                ofp[idx[i]:idx[i+1],:,:] = tempNorm.reshape(Np,Ny,Nz)
                del ofp


        #np.max(fp) #do in chunks
        self['norm_msidata'] = np.memmap(outputFilename, dtype='float', mode='w+', shape=(Nx,Ny,Nz))#ofp #.reshape(Nx,Ny,Nz)
        self['norm_mz'] = mzdata
        # del fp
        # inputFilename.close()
