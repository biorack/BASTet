...
def main(argv=None):
    ...
    ####################################################################
    if ConvertSettings.add_file_to_db:
        try:
            with warnings.catch_warnings(record=True) as w:
                warnings.simplefilter("always")
                status = ConvertWebHelper.register_file_with_db(filepath=omsiOutFile,
                                                                db_server=ConvertSettings.db_server_url,
                                                                file_owner_name=ConvertSettings.file_owner)
                ConvertSettings.recorded_warnings += w
        except ValueError:
    ...
    ####################################################################
    if len(ConvertSettings.recorded_warnings) == 0:
        ConvertWebHelper.send_email(subject='Conversion complete: '+str(omsiOutFile),
                                    body='Success',
                                    email_type='success')
    elif len(ConvertSettings.email_error_recipients) > 0:
        ConvertWebHelper.send_email(subject='Conversion completed with warnings: '+str(omsiOutFile),
                                    body=warningmsg,
                                    email_type='warning')
    ...
    def parse_input_args(cls, argv):
        ...
        while i < (len(argv) - 1):
            ...
            if currentArg == "--no-nmf":
                ...
                startIndex += 2
                ConvertSettings.file_owner = str(argv[i + 1])
                ...
                if errorOption in ConvertSettings.available_error_options:
                    ConvertSettings.error_handling = errorOption
                else:
        ...
        if (ConvertSettings.chunks is None) and (ConvertSettings.compression is not None):
            ...
            for i in range(numIter):
                ...
                #userInput = raw_input()
                userInput = UserInput.userinput_with_timeout(timeout=timeout, default=None)
...
class ConvertFiles(object):
    ...
    def convert_files():
        ...
        for i in ConvertSettings.dataset_list:  # xrange(startIndex,len(argv)-1):
            ####################################################################
            ...
            if ConvertSettings.auto_chunk:
                ...
                specChunks, imgChunks, tempChunks = ConvertFiles.suggest_chunking(
                    xsize=inputFile.shape[0],
                    ysize=inputFile.shape[1],
                    mzsize=inputFile.shape[2],
                    dtype=inputFile.data_type,
                    print_results=True)
                ConvertSettings.chunks = specChunks
                additionalChunks = ConvertSettings.user_additional_chunks + [imgChunks]
                ...
                additionalChunks = ConvertSettings.user_additional_chunks
            ...
            # Store this dataset in combination with the given experiment index
                ...
                exp = ConvertSettings.omsi_output_file.get_exp(i['exp'])
            ...
            try:
                if ConvertSettings.generate_thumbnail:
                    ...
                    if ConvertSettings.execute_nmf:
                        ...
                        thumbnailFilename = ConvertSettings.omsi_output_file.hdf_filename + \
                            "_" + expIndex + ".png"
                        ...
                        #expName = str(exp.get_h5py_experimentgroup().name)
                        #expIndex = expName[7:len(expName)]
                        #thumbnailFilename = ConvertSettings.omsi_output_file.hdf_filename+"_"+expIndex+".png"
                        #thumbnail.save(thumbnailFilename, 'PNG')
            except ImportError:
    ...
    def suggest_chunkings_for_files(in_dataset_list):
        ...
        """
        spectrumChunk = [()] * len(in_dataset_list)
        sliceChunk = [()] * len(in_dataset_list)
        balancedChunk = [()] * len(in_dataset_list)
        for di in range(len(in_dataset_list)):
            ...
            try:
                ...
                if currFormat is "img":
                    ...
                    continue
                print "In data shape: " + str(inputFile.shape)
                spectrumChunk[di], sliceChunk[di], balancedChunk[di] = ConvertFiles.suggest_chunking(
                    xsize=inputFile.shape[0],
                    ysize=inputFile.shape[1],
                    mzsize=inputFile.shape[2],
                    dtype=inputFile.data_type,
        ...
        if db_server == ConvertSettings.default_db_server_url and ConvertSettings.check_add_nersc:
            ...
            for ap in ConvertSettings.allowed_nersc_locations:
                if filepath.startswith(ap):
                    allowedpath = True
                    break
            if not allowedpath and file_owner_name in ConvertSettings.super_users:
